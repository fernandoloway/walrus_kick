...
app.get('/category/:id', function(req, res){
    ...
    `
    var sql2='select id as productId, productName, ProductImgName, CurrPrice from product_hd where productctgid=?;'
...
app.get('/product/:id', function(req, res){
    ...
    join seasonal_category B on b.id=a.seasonalctgid;
    `
    var sql2=`Select a.id as ProductId, ProductName, ProductImgName, ProductCtgId, ProductDesc, CurrPrice , ProductCtgName, SeasonalCtgName
    from product_hd A 
    ...
    connection.query (sql1+sql2+sql3+sql4, [req.params.id, req.params.id, req.params.id], function(err, rows, field){
        ...
        res.render(__dirname+'/views/user/product_page.ejs', { data:rows, season:distinctSeason, notif:notif, notifstatus, notifdisplay, uid:sess.uid});
...
app.get('/login', function(req, res){
    ...
    res.render(__dirname+'/views/user/user_login',{notif:'', visibility:'none', ref:req.query.ref});
    res.end();
});
...
app.get('/cart/', function(req, res){
    ...
    else
    {
        ...
        connection.query (sql1, sess.uid, function(err, rows, field){
            if (err) throw err;
            if (rows.length==0){
                res.render(__dirname+'/views/user/user_cart_kosong',{uid:sess.uid});
            }
            else {

                res.render(__dirname+'/views/user/user_cart',{data:rows, uid:sess.uid});
...
{   
    ...
    else
    {
        ...
        console.log(req.body.cart.id.length)
...
app.get('/checkout/', function(req, res){
    ...
    else
    {
        ...
        connection.query (sql1, sess.uid, function(err, rows, field){
            ...
            res.render(__dirname+'/views/user/checkout_form',{data:rows, uid:sess.uid});
            ...
            if (rows.length>0){
                ...
                connection.query(sql2, [sess.uid, req.body.name, req.body.zip, req.body.provinsi, req.body.kabupaten, req.body.address, req.body.phone, sess.uid ] , function(err, result){
                    ...
                    connection.query(sql3, [result.insertId, sess.uid ] , function(err, result){
                        if (err) throw err;
                        // console.log(result)
                        sql4=`Update product_dt A, user_cart B set a.stocks=a.stocks-b.quantity 
                        where userid=? and b.productdtid=a.id;`
                        sql5=`Delete from user_cart where userid=?;`
                        connection.query(sql4+sql5, [sess.uid, sess.uid] , function(err, result){
                            if (err) throw err;
                            res.render(__dirname+'/views/user/checkout_success',{uid:sess.uid});
                        });
                    });
...
app.get('/invoice/', function(req, res){
    ...
    else
    {
        ...
        connection.query (sql, sess.uid, function(err, rows, field){
            ...
            res.render(__dirname+'/views/user/user_invoice_hd',{data:rows, uid: sess.uid});
...
app.get('/invoice/:invoiceid', function(req, res){
    ...
    else
    {
        ...
        connection.query (sql, req.params.invoiceid, function(err, rows, field){
            ...
            res.render(__dirname+'/views/user/user_invoice_dt',{data:rows, uid:sess.uid});
...
app.get('/dashboard-invoice/', function(req, res){
    ...
    else
    {
        ...
        connection.query (sql, sess.uid, function(err, rows, field){
            ...
            res.render(__dirname+'/views/admin_dashboard/all_invoice',{data:rows, uid:sess.uid});
...
app.get('/dashboard-invoice-detail/:invoiceid', function(req, res){
    ...
    else
    {
        ...
        connection.query (sql, req.params.invoiceid, function(err, rows, field){
            ...
            res.render(__dirname+'/views/admin_dashboard/invoice_detail',{data:rows, uid:sess.uid});
